...
impl SettleAuctionNoneCctpShimData {
    pub fn from_bytes(data: &[u8]) -> Option<Self> {
        let cctp_message_bump = data[0];
        let auction_bump = data[1];
        Some(Self {
            cctp_message_bump,
            auction_bump,
        })
    ...
    pub fn to_bytes(&self) -> [u8; 2] {
        [self.cctp_message_bump, self.auction_bump]
    }
}
...
pub fn settle_auction_none_cctp_shim(
